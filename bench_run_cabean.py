...
re_bench = re.compile("\[v(\d+)\]__\[r(\d+)\]__\[(.+?)\]__\[(.+?)\].*\.bnet")
...
out_dir = "cabean_" + str(int(time.time()))
os.mkdir(out_dir)
...
for benchmark in benchmarks:
	...
	os.system("gtimeout 1h time ./CABEAN-release-2.0.0/BinaryFile/MacOS/cabean -compositional 2 " + in_file + " > " + out_file + " 2>&1")
	i = i + 1
