use crate::process::{ReachAfterPostProcess, FwdProcess, Process, Scheduler, ExtendedComponentProcess};
...
impl ReachAfterPostProcess {
    pub fn new(variable: VariableId, graph: &SymbolicAsyncGraph, universe: &GraphColoredVertices) -> ReachAfterPostProcess {
    ...
    fn step(&mut self, scheduler: &mut S, graph: &SymbolicAsyncGraph) -> bool {
        if self.fwd.step(scheduler, graph) {
            ...
            if fwd_set != scheduler.universe() {
                ...
                let basin = reach_bwd(
                    graph, scheduler.variables(), fwd_set, scheduler.universe()
                ).minus(fwd_set);
                if !basin.is_empty() {
            ...
            scheduler.spawn(ExtendedComponentProcess::new(
                self.variable, fwd_set, scheduler.universe(), graph
            ));
